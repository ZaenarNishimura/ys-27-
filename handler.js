...
export async function participantsUpdate({ id, participants, action }) {
    ...
    switch (action) {
        ...
        case 'remove':
            if (chat.welcome) {
                ...
                for (let user of participants) {
                    ...
                    try {
                        ...
                        text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'Selamat datang, @user').replace('@group', await this.getName(id)).replace('@desc', groupMetadata.desc?.toString() || 'Orang asing') :
            ...
            break
        case 'promote':
            text = (chat.sPromote || this.spromote || conn.spromote || '@user sekarang dia adalah admin!! Selamat!')
        case 'demote':
...
export async function groupsUpdate(groupsUpdate) {
    ...
    for (const groupUpdate of groupsUpdate) {
        ...
        if (groupUpdate.subject) text = (chats.sSubject || this.sSubject || conn.sSubject || 'Nama grup diubah a \n@group').replace('@subject', groupUpdate.subject)
        if (groupUpdate.icon) text = (chats.sIcon || this.sIcon || conn.sIcon || 'Ikon grup berubah').replace('@icon', groupUpdate.icon)
        if (groupUpdate.revoke) text = (chats.sRevoke || this.sRevoke || conn.sRevoke || 'Tautan grup berubah a\n@revoke').replace('@revoke', groupUpdate.revoke)
        if (!text) continue
...
global.dfail = (type, m, conn) => {
    let msg = {
        ...
        mods: '🔰  Fungsi ini hanya untuk *Untuk moderator Bot*',
        ...
        private: '📮 Perintah ini hanya bisa digunakan di chat *private Chat*',
        admin: '🛡️ Perintah ini hanya untuk *Admin Grup*',
        botAdmin: '💥 Untuk menggunakan perintah ini saya harus menjadi *Admini!*',
        unreg: '📇 Daftar untuk menggunakan fungsi ini Ketik:\n\n*/reg name.age*\n\n📌Contoh : */reg zky.21*',
