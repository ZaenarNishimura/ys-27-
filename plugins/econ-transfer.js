...
`.trim()
    ...
    if (!who) return m.reply('✳️ Tandai pengguna')
    if (!(who in global.db.data.users)) return m.reply(`✳️ Pengguna tidak ada di database saya`)
    if (user[type] * 1 < count) return m.reply(`✳️  *${type}*  tidak cukup untuk mentransfer`)
    let confirm = `
Anda yakin ingin mentransfer?? *${count}* _*${type}*_ a  *@${(who || '').replace(/@s\.whatsapp\.net/g, '')}* ? 

- Memiliki  *60s* 
_merespons *Dan* o *TIDAK*_
`.trim()
...
handler.before = async m => {
    ...
    if (/si?/g.test(m.text.toLowerCase())) {
        ...
        user[type] -= count * 1
        _user[type] += count * 1
        if (previous > user[type] * 1 && _previous < _user[type] * 1) m.reply(`✅ Pemindahan dilakukan \n\n*${count}* *${type}*  a @${(to || '').replace(/@s\.whatsapp\.net/g, '')}`, null, { mentions: [to] })
        else {
            user[type] = previous
