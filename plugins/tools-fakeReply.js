...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  if (!text) return m.reply(`✳️ Penggunaan Perintah\n\n*${usedPrefix + command}* Hai apa? @${m.sender.split`@`[0]} tidak ada dan kamu`, null, { mentions: [m.sender] })
  let cm = copy(m)
  let who
  if (text.includes('@0')) who = '0@s.whatsapp.net'
  else if (m.isGroup) who = cm.participant = m.mentionedJid[0]
  else who = m.chat
  if (!who) return m.reply(`✳️ Penggunaan Perintah\n\n*${usedPrefix + command}* Hai apa? @${m.sender.split`@`[0]} tidak ada dan Anda`, null, { mentions: [m.sender] })
  cm.key.fromMe = false 
